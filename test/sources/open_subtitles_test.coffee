...
describe "Open Subtitles Source", ->
  ...
  describe "#search", ->
    it "returns blank list when no subtitle is found", (source, api) ->
      source.hashQuery = quickStub(path, lang, W "query")
      api.search = quickStub(["query"], W data: null)

      expect(source.search(path, lang)).eql []

    it "returns an item from the result as a subtitle instance", (source, api) ->
      source.hashQuery = quickStub(path, lang, W "query")

      api.search = quickStub(["query"], W data: ['sub'])

      source.search(path, lang).then ([subtitle]) ->
        expect(subtitle.info).eql 'sub'
